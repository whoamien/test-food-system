...
export class AutoLoginController {
    ...
    async autoLoginUser(req: Request, res: Response): Promise<Response> {
        ...
        return res.status(200).json({ message: "Auto-login successful.", user: foundUser });
    }

    // Auto-login for admin users
    async autoLoginAdmin(req: Request, res: Response): Promise<Response> {
        const adminUser = (req as any).user; // Extracted from the token by authMiddleware

        if (!adminUser) {
            return res.status(401).json({ message: "Unauthorized." });
        }

        const foundAdmin = await this.adminUserRepository.findOneBy({ id: adminUser.id });

        if (!foundAdmin) {
            return res.status(404).json({ message: "Admin user not found." });
        }

        return res.status(200).json({ message: "Auto-login successful.", adminUser: foundAdmin });
    }
}